 ...
 */
public abstract class ExcelReader2<T> {
    ...
    private List<T> parseExcel(Workbook workbook) {
        ...
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                T resultData = (T) convertRowToData(row);
                if (null == resultData) {
                    errorRow++;
        ...
        logger.warning("共计 "+errorRow+" 行数据不合法,已忽略");
        return resultDataList;
    }
     ...
     */
    protected abstract ExcelDataVO convertRowToData(Row row);
}
